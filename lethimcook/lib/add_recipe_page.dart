...
class AddRecipePage extends StatefulWidget {
  /// Optionally pre-fill the form with an imported recipe
  final Recipe? initialRecipe;
  const AddRecipePage({super.key, this.initialRecipe});
...
class _AddRecipePageState extends State<AddRecipePage> {
  ...
  late int servings;
  late int prepTime; // minutes, 0 = non défini
  late int restTime;
  late int cookTime;
  ...
  void initState() {
    ...
    selectedType = r?.type ?? 'plat';
    ingredients = List.from(r?.ingredients ?? []);
    steps = List.from(r?.steps ?? []);
    remarks = List.from(r?.remarks ?? []);
  ...
  Future<void> _pickImage() async {
    ...
    if (cropped != null) setState(() => imagePath = cropped.path);
  }
  ...
  void _save() async {
    ...
    try {
      final recipe = Recipe(
        ...
        restTime: restTime > 0 ? restTime : null,
                ...
                children: [
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      prefixIcon:
                          Icon(Icons.restaurant_menu, color: kTextSecondary),
                    ),
                  ),
...
class _TimePickerRow extends StatelessWidget {
  ...
  final String label;
  final Color color;
  final int value; // minutes, 0 = non défini
